...
// ============================================================
//  MOSTRAR ENTRADA COMPLETA (SIN CONCATENACIÓN DE HTML CON ONCLICK)
// ============================================================
function mostrarEntradaCompleta(entrada, esMovil) {
    ...
    // HTML BASE (seguro porque no tiene onclick)
    var htmlBase = '<h2>' + entrada.aaa + '</h2>';
    htmlBase += '<div class="contenido-bbb">' + procesarContenidoBBB(entrada.bbb) + '</div>';

    targetEl.innerHTML = htmlBase;

    // CREAR ETIQUETAS CON DOM
    var etiquetas = entrada.ccc.split(',').map(function(e) { return e.trim(); }).filter(function(e) { return e; });
    
    if (etiquetas.length > 0) {
        var etiquetasDiv = document.createElement('div');
        etiquetasDiv.className = 'etiquetas-completas';
        
        var tituloEtq = document.createElement('h4');
        tituloEtq.textContent = 'Etiquetas';
        etiquetasDiv.appendChild(tituloEtq);
        ...
        etiquetas.forEach(function(etq) {
            ...
            var li = document.createElement('li');
            li.textContent = etqLimpia;
            li.setAttribute('data-etiqueta', etqLimpia);
            
            if (esPrincipal) {
                li.style.fontWeight = '700';
                li.style.color = '#b0c4d8';
            }
            
            li.addEventListener('click', function() {
                var etiqueta = this.getAttribute('data-etiqueta');
        ...
        etiquetasDiv.appendChild(listaUl);
        
        var contenidoBbb = targetEl.querySelector('.contenido-bbb');
        if (contenidoBbb) {
            contenidoBbb.parentNode.insertBefore(etiquetasDiv, contenidoBbb.nextSibling);
        } else {
            targetEl.appendChild(etiquetasDiv);
        }
    }

    // CREAR SECCIÓN DE COMPARTIR CON DOM
    ...
    var compartirDiv = document.createElement('div');
    compartirDiv.className = 'compartir';
    
    var tituloComp = document.createElement('h4');
    tituloComp.textContent = 'Compartir';
    compartirDiv.appendChild(tituloComp);
    
    var botonesDiv = document.createElement('div');
    botonesDiv.className = 'botones-compartir';
    
    // Botón Enlace
    var btnEnlace = document.createElement('span');
    btnEnlace.className = 'btn-compartir';
    btnEnlace.textContent = 'Enlace';
    btnEnlace.setAttribute('data-id', entrada.ddd);
    btnEnlace.setAttribute('data-accion', 'enlace');
    btnEnlace.addEventListener('click', function() {
        var id = this.getAttribute('data-id');
        copiarEnlacePermanente(id, this);
    });
    botonesDiv.appendChild(btnEnlace);
    
    // Botón Copiar
    var btnCopiar = document.createElement('span');
    btnCopiar.className = 'btn-compartir';
    btnCopiar.textContent = 'Copiar';
    btnCopiar.setAttribute('data-texto', textoCopiar);
    btnCopiar.setAttribute('data-accion', 'copiar');
    btnCopiar.addEventListener('click', function() {
        var texto = this.getAttribute('data-texto');
        copiarAlPortapapeles(texto, this);
    });
    botonesDiv.appendChild(btnCopiar);
    
    // Botón QR
    var btnQr = document.createElement('span');
    btnQr.className = 'btn-compartir';
    btnQr.textContent = 'QR';
    btnQr.setAttribute('data-id', entrada.ddd);
    btnQr.setAttribute('data-qrid', qrId);
    btnQr.setAttribute('data-accion', 'qr');
    btnQr.addEventListener('click', function() {
        var id = this.getAttribute('data-id');
        var qrId = this.getAttribute('data-qrid');
        mostrarQR(id, qrId, this);
    });
    botonesDiv.appendChild(btnQr);
    
    // Botón Tuitear
    var btnTweet = document.createElement('a');
    btnTweet.className = 'btn-compartir';
    btnTweet.textContent = '𝕏 Tuitear ↗';
    btnTweet.href = tweetUrl;
    btnTweet.target = '_blank';
    btnTweet.rel = 'noopener noreferrer';
    botonesDiv.appendChild(btnTweet);
    
    compartirDiv.appendChild(botonesDiv);
    
    // Contenedor QR
    var qrContainer = document.createElement('div');
    qrContainer.id = qrId;
    qrContainer.style.display = 'none';
    qrContainer.style.marginTop = '18px';
    qrContainer.style.padding = '0';
    qrContainer.style.borderRadius = '7px';
    qrContainer.style.textAlign = 'center';
    qrContainer.style.background = 'transparent';
    compartirDiv.appendChild(qrContainer);
    
    // Insertar la sección de compartir
    var etiquetasCompletas = targetEl.querySelector('.etiquetas-completas');
    if (etiquetasCompletas) {
        etiquetasCompletas.parentNode.insertBefore(compartirDiv, etiquetasCompletas.nextSibling);
    } else {
        var contenidoBbb2 = targetEl.querySelector('.contenido-bbb');
        if (contenidoBbb2) {
            contenidoBbb2.parentNode.insertBefore(compartirDiv, contenidoBbb2.nextSibling);
        } else {
            targetEl.appendChild(compartirDiv);
        }
    }
}
...
function mostrarQR(id, containerId, elementoBoton) {
    ...
    small.textContent = 'Escanea para abrir esta entrada';
    small.style.display = 'block';
    small.style.marginTop = '4px';
    small.style.color = '#8a9aaa';
